      ...
      dealNormal: function (data) {
        ...
        for (let index in data) {
          ...
          if (metricsDTO['metricLevel'] === 'NORMAL') {
            ...
            let ipAddress = metricsDTO['ip']
            let isProvider = metric.split('.')[1]
            metric = isProvider + '.' + metric.substring(metric.lastIndexOf('.') + 1)
            let methodMap = serviceMethodMap[metricsDTO.tags.service +'##'+ ipAddress]
            if (!methodMap) {
              methodMap = {}
              serviceMethodMap[metricsDTO.tags.service +'##'+ ipAddress] = methodMap
            }
            let metricMap = methodMap[metricsDTO.tags.method]
            if (!metricMap) {
              metricMap = {}
              serviceMethodMap[metricsDTO.tags.service +'##'+ ipAddress][metricsDTO.tags.method] = metricMap
            }
        ...
        for (let service in serviceMethodMap) {
          for (let method in serviceMethodMap[service]) {
            ...
            let tmp = service.split("##")
            let realService = tmp[0]
            let tempIp =  tmp[1]
            this.addDataToDetails(this.providerDetails, realService, method, metricsMap, tempIp, 'provider')
            this.addDataToDetails(this.consumerDetails, realService, method, metricsMap, tempIp, 'consumer')
          }
        }
      },
      addDataToDetails: function (sideDetails, service, method, metricsMap, ip, side) {
